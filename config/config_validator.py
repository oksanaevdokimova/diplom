...
def _validate_gsm(gsm: dict[str, Any]) -> None: # gsm — словарь секции gsm из конфигурации
    ...
    if mode == "tcp": # Если tcp
        ...
        return
    raise ValueError(f"gsm.mode должен быть 'tcp' или 'mqtt', получено: {mode!r}")

"""Проверка всей конфигурации перед загрузкой или сохранением"""
def validate_config(config: dict[str, Any]) -> None: # config — словарь конфигурации
    ...
    _require_positive_int("default_speed", config.get("default_speed"))
    ...
    if transport == "usb": # Если usb
        ...
        raise ValueError(
            f"active_transport должен быть usb, wifi или gsm, получено: {transport!r}",
        )
