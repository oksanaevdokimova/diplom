...
def create_transport(config: dict[str, Any], parent: Any = None) -> BaseTransport:
    ...
    if active == "wifi": # Если активный транспорт WiFi
        ...
        return TcpTransport(host=str(wifi["host"]), port=int(wifi["port"]), parent=parent) # Создание транспорта TCP
    if active == "gsm": # Если активный транспорт GSM
        ...
        if mode == "tcp": # Если режим TCP
            return TcpTransport(host=str(gsm["host"]), port=int(gsm["port"]), parent=parent) # Создание транспорта TCP
        if mode == "mqtt":
            return MqttTransport(broker_host=str(gsm["broker_host"]), broker_port=int(gsm["broker_port"]), topic_command=str(gsm["topic_command"]), topic_messages=str(gsm["topic_messages"]), parent=parent) # Создание транспорта MQTT
